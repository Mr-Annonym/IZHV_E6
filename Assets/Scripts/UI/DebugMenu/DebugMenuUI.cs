    ...
    private void MainWindowUI(int windowId)
    {
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                interactive = GUILayout.Toggle(interactive, "", GUILayout.Width(24));
                if (interactive != GameManager.Instance.interactiveMode)
                ...
                if (masterVolume != SoundManager.Instance.masterVolume)
                    SoundManager.Instance.masterVolume = masterVolume;
                // Compact mute button
                bool muted = SoundManager.Instance.masterMuted;
                string muteLabel = muted ? "M" : "M";
                bool newMuted = GUILayout.Toggle(muted, muteLabel, "Button", GUILayout.Width(24));
                if (newMuted != muted)
                    SoundManager.Instance.masterMuted = newMuted;
                GUILayout.EndHorizontal();

                // --- Dummy Sliders & Button ---
                GUILayout.BeginHorizontal();
                ...
                if (GUILayout.Button("Enable\nDummy\nCharacter", GUILayout.Width(96), GUILayout.ExpandHeight(true)))
                {
